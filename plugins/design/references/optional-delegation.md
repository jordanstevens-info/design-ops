...
`design` does not package custom agents. The public plugin contract is
...
Design review and eval are the strongest cases for delegation. A fresh-context
reviewer can avoid inheriting the primary agent's implementation rationale or
...
Fresh-context review is preferred when the primary agent authored, generated, or
materially changed the artifact being evaluated, especially for design-system
acceptance scenarios or full evals. Keep it optional and do not block if the
runtime cannot delegate.

For smoke evals, give the reviewer only the smoke question, target artifact,
expected outcome, relevant acceptance criteria or rubric, highest known risk,
and data handling boundary. For full evals, include frozen input/output refs and
the named rubric/profile, but not the primary agent's defense or intended fix.
