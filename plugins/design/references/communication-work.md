...
2. If `.design/artifacts/`, `.design/demos/`, or `.design/presos/` exists, scan for relevant
   additional context.
...
If the remote host is private, draft the URL inline only; ask before publishing,
posting, or sharing it outside the current response.
...
| `chore/` | `:broom:` | Chore |
| `docs/` | `:books:` | Docs |
| `plugin/` | `:electric_plug:` | Plugin Dev |
| no prefix or unknown | `:memo:` | Update |

## Message Format

```markdown
**<Emoji> <Update Label> ~ <Feature Name> (AB-1234)**
...
- `:rainbow: Design Update ~ Market Configuration`
- `:sparkles: Feature Update ~ Reserved Seating`
- `:memo: Update ~ Stakeholder Readout`
...
| Update Label | Use `<Category> Update`, such as `Design Update`, when the category is specific. Use `Update` when the category is already `Update`. |
