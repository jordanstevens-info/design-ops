...
- Start from the artifact state and entry mode before editing.
- Apply data-handling rules before processing source material or evidence.
...
- Design-system acceptance is an implementation gate when a target profile
  exists.
...
- Prefer project-owned tooltip or component primitives over third-party defaults
  when the repo provides them.
...
- Create a new function, type, utility, or component without searching the repo
  first.
- Process known or suspected C4 data with AI. Stop as `Prohibited` instead.
...
12. If a git remote exists, ask for explicit user approval before pushing the
    design branch to remote. Do not open or create an MR.
